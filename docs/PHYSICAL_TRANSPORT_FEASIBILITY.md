...
| --- | --- | --- |
| Supported host boundary | PhoneAPI over BLE, serial, TCP, or HTTP; structural active-SDK-compatible port plus an isolated pinned serial bench | Native application's internal `Transport` abstraction; no documented external arbitrary-payload API found |
...
- `MeshtasticSdkFramePort` maps the shared `sendPacket` and `onPrivatePacket` surface. The GPL runtime exists only in the private pinned serial-bench subpackage.
